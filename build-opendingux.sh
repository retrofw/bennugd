...
TARGET=mipsel-linux
BUILD=`uname -m`
PKG_CONFIG_PATH=/home/steward/Downloads/buildroot-2017.02.9/output/host/usr/mipsel-buildroot-linux-uclibc/sysroot/usr/lib/pkgconfig
...
case $1 in
    release)
        echo "./configure --prefix=${PREFIX} --target=${TARGET} --host=${HOST} --build=${BUILD} --enable-shared PKG_CONFIG_LIBDIR=${PKG_CONFIG_PATH} && make clean && make"
...
case $1 in
    release)
        echo "./configure --prefix=${PREFIX} --target=${TARGET} --host=${HOST} --build=${BUILD} --enable-shared PKG_CONFIG_LIBDIR=${PKG_CONFIG_PATH} && make clean && make"
        ./configure --prefix=${PREFIX} --target=${TARGET} --host=${HOST} --build=${BUILD} --enable-shared PKG_CONFIG_PATH=${PKG_CONFIG_PATH} PNG_LIBS="-llibpng" && make clean && make
        ;;
